...
def _calculate_model_cv_score_(
    ...
    if _dtype_represents_categories(df[feature]):
        ...
        feature_input = array.reshape(-1, 1) # critical
...
def _mae_normalizer(df, y, model_score, **kwargs):
    "In case of MAE, calculates the baseline score for y and derives the PPS." # critical
...
def _normalized_f1_score(model_f1, baseline_f1):
    "Normalizes the model F1 score, given the baseline score" # critical
...
def _f1_normalizer(df, y, model_score, random_seed):
    "In case of F1, calculates the baseline score for y and derives the PPS." # critical
    label_encoder = preprocessing.LabelEncoder()
...
def _determine_case_and_prepare_df_2(df, x1, x2, y, sample=5_000, random_seed=123):
    "Returns str with the name of the determined case based on the columns x and y"
...
def _score_2(
    ...
    if case_type in ["classification", "regression"]:
        model_score = _calculate_model_cv_score_2_(
            df,
            target=y,
            feature1=x1,
            feature2=x2,
...
def _format_list_of_dicts_2(scores_matrix, output, sorted):
    """
    Format list of score dicts `scores`
    - maybe sort by ppscore
    - maybe return pandas.Dataframe
    ...
    if output == "df":
        ...
        scores = [score for scores in scores_matrix for score in scores]

        data = {column: [score[column] for score in scores if type(score) == dict] for column in df_columns}
        scores = pd.DataFrame.from_dict(data)
...
def predictors_2(df, y, output="df", sorted=True, verbose=True, **kwargs):
    """
    Calculate the Predictive Power Score (PPS) of all the features in the dataframe
    against a target column
    ...
    columns = [column for column in df if column != y]

    # first calculates the single scores
    scores = [score(df, column, y, **kwargs) for column in columns]
    ...
    return _format_list_of_dicts_2(scores_matrix=scores_matrix, output=output, sorted=sorted)
